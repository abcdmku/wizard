...
<Steps>
    ...
    const steps = defineSteps({
      ...
      user: step({
        ...
        canEnter: ({ context, data }) => {
          // ✅ context is automatically typed as CheckoutContext
          // ✅ data is automatically typed as { name: string; email: string; password: string } | undefined
  ...
  <Step title="Create and use your wizard">
    ...
    // ✅ TypeScript knows this is { name: string; email: string; password: string } | undefined
    const userData = currentStep.data;
    if (userData) {
      console.log('User email:', userData.email); // ✅ Fully typed!
    }

    // Navigate with type-safe step names
    await wizard.next();
    await wizard.goTo('payment'); // ✅ TypeScript validates step names
    await wizard.back();
...
function CheckoutWizard() {
  // Use the kitchen sink hook to get everything
  const { step, helpers, next, back } = useWizard(wizard);

  const progress = helpers.progress();
  // Call helpers directly or use extracted methods
  const canGoNext = helpers.canGoNext();
...
function UserStep() {
  // Use useWizardStep for specific step data/actions
  const { data, setStepData, next } = useWizardStep(wizard, 'user');
...
- **🎯 Automatic Type Inference**: Context and step data types are automatically inferred throughout your application
- **🔒 Compile-time Safety**: Invalid step names, data shapes, and context mutations are caught at compile time
- **🚀 Modern Patterns**: Uses the latest factory pattern with `wizardWithContext` for the best developer experience
- **⚡ Performance**: Optimized type system provides excellent IDE support and fast compilation
- **🔄 Fluent API**: Method chaining with `.getStep('name').setData().markLoading()` patterns
...
- Read the [API Reference](/api-docs) for detailed documentation