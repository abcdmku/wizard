...
WizardOpus distinguishes between **runtime status** (explicitly set) and **computed status** (derived from state).
  ...
  steps: {
    payment: {
      load: async (data) => {
        ...
        // idle → loading (when async starts)
        try {
...
```typescript
import { createStatusMarkers } from '@wizard/core/internal';

class CustomWizardController {
  private markers: StatusMarkers<Steps>;

  constructor(wizard: Wizard<C, Steps, D>) {
    // Note: This uses internal APIs - be careful with updates
    this.markers = createStatusMarkers(wizard.config, wizard.state);
  }

  // Manually mark step as loading
  setLoading(step: Steps) {
    this.markers.markLoading(step);
  }

  // Mark step as errored
  setError(step: Steps, error: Error) {
    this.markers.markError(step, error);
  }

  // Mark step as terminated
  terminate(step: Steps, reason?: string) {
    this.markers.markTerminated(step, new Error(reason || 'Terminated'));
  }

  // Mark step as skipped
  skip(step: Steps) {
    this.markers.markSkipped(step);
  }

  // Clear status (back to idle)
  clearStatus(step: Steps) {
    this.markers.markIdle(step);
  }
}
```
...
function StepStatusIndicator({ step }: { step: string }) {
  ...
  const statusConfig = {
    current: { icon: '→', color: 'blue', label: 'Current' },
    completed: { icon: '✓', color: 'green', label: 'Complete' },
    error: { icon: '✗', color: 'red', label: 'Error' },
    loading: { icon: '⟳', color: 'orange', label: 'Loading' },
    skipped: { icon: '⤳', color: 'gray', label: 'Skipped' },
    unavailable: { icon: '🔒', color: 'gray', label: 'Locked' },
    required: { icon: '●', color: 'yellow', label: 'Required' },
    optional: { icon: '○', color: 'gray', label: 'Optional' },
    terminated: { icon: '⊗', color: 'red', label: 'Terminated' }
  };
  ...
  return (
    <div className="detailed-progress">
      <div className="status-summary">
        ...
        <div>⟳ In Progress: {statusCounts.loading || 0}</div>
        <div>✗ Errors: {statusCounts.error || 0}</div>
...
- Explore [DAG Prerequisites](/advanced/dag-prerequisites) for complex flows
- Master [Dynamic Flows](/advanced/dynamic-flow) for conditional logic