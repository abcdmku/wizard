...
This example uses the **factory pattern** with `createWizardFactory()`:

```typescript file="@examples/basic-form-wizard/src/wizard/steps.ts" lines="40-68"
```

<Callout type="info">
  Both `createWizardFactory()` and `wizardWithContext()` are valid patterns. The latter is a convenience wrapper that pre-binds context.
</Callout>

### Alternative: Using `wizardWithContext`

Here's how you could rewrite this using the `wizardWithContext` convenience helper:
